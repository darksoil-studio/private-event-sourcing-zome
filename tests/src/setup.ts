...
const testHappUrl = dirname(fileURLToPath(import.meta.url)) + '/../../workdir/private-event-sourcing_test.happ';
...
async function addPlayer(scenario: Scenario) {
  ...
  const store = new PrivateEventSourcingStore(
  	new PrivateEventSourcingClient(player.appWs as any, 'private_event_sourcing_test'),
  );
  return {
  	store,
  	player,
